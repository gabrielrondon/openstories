...
- scenario: EV charger loses cellular modem connection during active charging session
    on carbon-accounting combined with Cold-Start Latency & Resource Starvation
...
- Emergency stop button pressed during offline session requiring local safety cut-off
  within 100ms%!(EXTRA string=carbon-accounting) exacerbated by Cold-Start Latency
  & Resource Starvation
