...
story:
  ...
  i_want: bidirectional OCPP messaging with offline transaction caching for ev-charging-protocols
    EV charge points with resilience to Zero-Trust Authentication & Token Invalidation
...
- scenario: EV charger loses cellular modem connection during active charging session
    on ev-charging-protocols combined with Zero-Trust Authentication & Token Invalidation
  given: An active high-power DC fast charging session delivering 150 kW
  when: The station's cellular uplink drops%!(EXTRA string=ev-charging-protocols)
...
- Emergency stop button pressed during offline session requiring local safety cut-off
  within 100ms%!(EXTRA string=ev-charging-protocols) exacerbated by Zero-Trust Authentication
  & Token Invalidation
...
- Does the implementation mitigate emergency stop button pressed during offline session
  requiring local safety cut-off within 100ms%!(extra string=ev-charging-protocols)
