...
- scenario: EV charger loses cellular modem connection during active charging session
    on ev-charging-protocols combined with Cold-Start Latency & Resource Starvation
...
- Emergency stop button pressed during offline session requiring local safety cut-off
  within 100ms%!(EXTRA string=ev-charging-protocols) exacerbated by Cold-Start Latency
  & Resource Starvation
