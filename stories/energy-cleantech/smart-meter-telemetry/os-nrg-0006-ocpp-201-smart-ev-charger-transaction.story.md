...
- scenario: EV charger loses cellular modem connection during active charging session
    on smart-meter-telemetry combined with Cold-Start Latency & Resource Starvation
...
- Emergency stop button pressed during offline session requiring local safety cut-off
  within 100ms%!(EXTRA string=smart-meter-telemetry) exacerbated by Cold-Start Latency
  & Resource Starvation
