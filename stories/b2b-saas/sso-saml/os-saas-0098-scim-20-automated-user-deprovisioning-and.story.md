...
industry: b2b-saas
domain: sso-saml
title: SCIM 2.0 automated user deprovisioning and role sync for sso-saml under Asynchronous
...
story:
  ...
  i_want: full compliance with RFC 7644 SCIM protocol for sso-saml user life-cycle
    management with resilience to Asynchronous Race Conditions & Deadlocks
...
- User reassigned to a different department with reduced permissions while currently
  holding an active session exacerbated by Asynchronous Race Conditions & Deadlocks
- Cascading failover during Asynchronous Race Conditions & Deadlocks
evidence:
- source: https://github.com/boxyhq/jackson/issues/612
  type: production_incident_report
  quote: We failed an enterprise procurement audit because our sso-saml system didn't
...
- Does the implementation mitigate user reassigned to a different department with
  reduced permissions while currently holding an active session exacerbated by asynchronous
