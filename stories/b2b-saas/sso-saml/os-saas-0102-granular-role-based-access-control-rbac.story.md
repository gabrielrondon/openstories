...
industry: b2b-saas
domain: sso-saml
title: Granular Role-Based Access Control (RBAC) with attribute constraints for sso-saml
...
story:
  ...
  i_want: fine-grained policy evaluation (ABAC/RBAC) on all sso-saml endpoints with
    resilience to Network Partitions & Distributed Timeout Failures
...
- Users belonging to multiple organizations switching active workspace context concurrently%!(EXTRA
  string=sso-saml) exacerbated by Network Partitions & Distributed Timeout Failures
- Cascading failover during Network Partitions & Distributed Timeout Failures
evidence:
- source: https://hackerone.com/reports/512091
  type: production_incident_report
  quote: A classic IDOR vulnerability in our sso-saml module allowed team members
...
- Does the implementation mitigate users belonging to multiple organizations switching
  active workspace context concurrently%!(extra string=sso-saml) exacerbated by network
