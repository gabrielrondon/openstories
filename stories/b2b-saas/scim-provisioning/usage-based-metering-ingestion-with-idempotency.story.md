...
title: Usage-based metering ingestion with idempotency and late-arriving event processing
  for scim-provisioning under Disaster Recovery & Cascading Failover
demand_score: 8.83
status: verified
persona:
...
- Client replay of telemetry batches leading to double-counting of billable compute
  metrics%!(EXTRA string=scim-provisioning) exacerbated by Disaster Recovery & Cascading
  Failover
...
- Does the implementation mitigate client replay of telemetry batches leading to double-counting
  of billable compute metrics%!(extra string=scim-provisioning) exacerbated by disaster
