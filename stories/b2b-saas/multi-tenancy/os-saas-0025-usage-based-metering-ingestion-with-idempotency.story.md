...
title: Usage-based metering ingestion with idempotency and late-arriving event processing
  for multi-tenancy under Multi-Tenant Data Leakage & Isolation Breaches
demand_score: 8.88
status: verified
persona:
...
- Client replay of telemetry batches leading to double-counting of billable compute
  metrics%!(EXTRA string=multi-tenancy) exacerbated by Multi-Tenant Data Leakage &
  Isolation Breaches
...
- Does the implementation mitigate client replay of telemetry batches leading to double-counting
  of billable compute metrics%!(extra string=multi-tenancy) exacerbated by multi-tenant
