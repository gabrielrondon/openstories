...
title: Usage-based metering ingestion with idempotency and late-arriving event processing
  for audit-logs under Network Partitions & Distributed Timeout Failures
demand_score: 9.07
status: verified
persona:
...
story:
  ...
  i_want: event-driven usage aggregation for audit-logs supporting out-of-order event
    streams with resilience to Network Partitions & Distributed Timeout Failures
...
- scenario: Network blip causes batch of usage events for audit-logs to arrive after
    monthly invoice finalization combined with Network Partitions & Distributed Timeout
    Failures
  given: The billing cycle closed on midnight of the 1st
  when: Usage metrics timestamped for the 31st arrive 6 hours late%!(EXTRA string=audit-logs)
...
- Client replay of telemetry batches leading to double-counting of billable compute
  metrics%!(EXTRA string=audit-logs) exacerbated by Network Partitions & Distributed
  Timeout Failures
...
- Does the implementation mitigate client replay of telemetry batches leading to double-counting
  of billable compute metrics%!(extra string=audit-logs) exacerbated by network partitions
