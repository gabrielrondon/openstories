...
title: Usage-based metering ingestion with idempotency and late-arriving event processing
  for usage-billing under Network Partitions & Distributed Timeout Failures
demand_score: 9.07
status: verified
persona:
...
- Client replay of telemetry batches leading to double-counting of billable compute
  metrics%!(EXTRA string=usage-billing) exacerbated by Network Partitions & Distributed
  Timeout Failures
...
- Does the implementation mitigate client replay of telemetry batches leading to double-counting
  of billable compute metrics%!(extra string=usage-billing) exacerbated by network
