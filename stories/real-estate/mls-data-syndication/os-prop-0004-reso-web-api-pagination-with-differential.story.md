...
domain: mls-data-syndication
title: RESO Web API pagination with differential delta synchronization for mls-data-syndication
...
story:
  ...
  i_want: replication-state tracking and delta synchronization for mls-data-syndication
    property listings with resilience to Strict Compliance & Regulatory Audit Enforcement
  so_that: property search portals update price drops and pending sale statuses within
    60 seconds of MLS changes
acceptance_criteria:
- scenario: MLS server drops pagination token during large 50k listing pull for mls-data-syndication
...
- Listings deleted or marked private by agents leaving phantom listings active on
  public search%!(EXTRA string=mls-data-syndication) exacerbated by Strict Compliance
...
- source: https://github.com/reso-standards/reso-web-api/issues/102
  ...
  quote: Our mls-data-syndication sync dropped pending status updates, showing houses
...
- Does the implementation mitigate listings deleted or marked private by agents leaving
  phantom listings active on public search%!(extra string=mls-data-syndication) exacerbated
