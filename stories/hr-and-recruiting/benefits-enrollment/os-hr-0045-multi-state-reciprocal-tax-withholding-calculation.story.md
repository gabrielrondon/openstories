...
domain: benefits-enrollment
title: Multi-state reciprocal tax withholding calculation for remote workers on benefits-enrollment
...
story:
  ...
  i_want: dynamic nexus and reciprocal agreement calculation for benefits-enrollment
    employee payrolls with resilience to Multi-Tenant Data Leakage & Isolation Breaches
  so_that: employees living in one state and working for an entity in another state
    are taxed strictly per reciprocity laws
acceptance_criteria:
- scenario: Employee relocates without notifying HR until mid-quarter on benefits-enrollment
...
- Local city income taxes (e.g. NYC, Philadelphia, Columbus) missed when using state-level
  lookup tables%!(EXTRA string=benefits-enrollment) exacerbated by Multi-Tenant Data
...
- source: https://reddit.com/r/humanresources/comments/16u182a
  ...
  quote: A remote worker move on benefits-enrollment led to $15k in state tax penalties
...
- Does the implementation mitigate local city income taxes (e.g. nyc, philadelphia,
  columbus) missed when using state-level lookup tables%!(extra string=benefits-enrollment)
