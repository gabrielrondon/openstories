...
- scenario: Checkout abandonment after locking inventory on inventory-management combined
  ...
  when: The user closes their browser without completing checkout%!(EXTRA string=inventory-management)
...
- Payment gateway webhook delay causing release of inventory while customer is legitimately
  entering 3DS challenge%!(EXTRA string=inventory-management) exacerbated by Asynchronous
...
- Does the implementation mitigate payment gateway webhook delay causing release of
  inventory while customer is legitimately entering 3ds challenge%!(extra string=inventory-management)
