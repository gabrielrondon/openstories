...
title: Multi-stack coupon code validation preventing promotional stacking abuse on
  promotions-and-coupons under Idempotency & Replay Attack Vulnerabilities
demand_score: 9.07
status: verified
persona:
...
story:
  ...
  i_want: strict rules engine governing coupon combinability and minimum order values
    on promotions-and-coupons with resilience to Idempotency & Replay Attack Vulnerabilities
...
- Customers creating multiple throwaway accounts with the same physical delivery address
  to bypass limits exacerbated by Idempotency & Replay Attack Vulnerabilities
...
- Does the implementation mitigate customers creating multiple throwaway accounts
  with the same physical delivery address to bypass limits exacerbated by idempotency
