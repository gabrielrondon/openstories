...
title: Multi-stack coupon code validation preventing promotional stacking abuse on
  cart-and-checkout under Network Partitions & Distributed Timeout Failures
demand_score: 9.22
status: verified
persona:
...
story:
  ...
  i_want: strict rules engine governing coupon combinability and minimum order values
    on cart-and-checkout with resilience to Network Partitions & Distributed Timeout
    Failures
...
- Customers creating multiple throwaway accounts with the same physical delivery address
  to bypass limits%!(EXTRA string=cart-and-checkout) exacerbated by Network Partitions
