...
story:
  ...
  i_want: a resilient sliding-window rate limiter for auth-and-iam with Redis fallback
    to local memory with resilience to Disaster Recovery & Cascading Failover
...
- Clock drift between distributed nodes skewing sliding window timestamp calculations%!(EXTRA
  string=auth-and-iam) exacerbated by Disaster Recovery & Cascading Failover
...
- Does the implementation mitigate clock drift between distributed nodes skewing sliding
  window timestamp calculations%!(extra string=auth-and-iam) exacerbated by disaster
