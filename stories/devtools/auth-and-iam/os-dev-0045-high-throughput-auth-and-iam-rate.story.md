...
industry: devtools
domain: auth-and-iam
title: High-throughput auth-and-iam rate limiting with distributed sliding window
...
story:
  ...
  i_want: a resilient sliding-window rate limiter for auth-and-iam with Redis fallback
    to local memory with resilience to Multi-Tenant Data Leakage & Isolation Breaches
  so_that: spiky traffic bursts do not exhaust internal worker pools or cascade into
    504 gateway timeouts
acceptance_criteria:
- scenario: Redis cluster unreachable during rate limit evaluation for auth-and-iam
    combined with Multi-Tenant Data Leakage & Isolation Breaches
  given: The central Redis cache drops connection
  when: Incoming client requests arrive at auth-and-iam
...
- Clock drift between distributed nodes skewing sliding window timestamp calculations%!(EXTRA
  string=auth-and-iam) exacerbated by Multi-Tenant Data Leakage & Isolation Breaches
...
- source: https://news.ycombinator.com/item?id=38190211
  ...
  quote: When Redis had a network partition, our auth-and-iam rate limiter crashed
    the entire microservice fleet.
  date: 2024-2025
  platform: ""
evaluation_rubric:
- Does the implementation mitigate clock drift between distributed nodes skewing sliding
  window timestamp calculations%!(extra string=auth-and-iam) exacerbated by multi-tenant
