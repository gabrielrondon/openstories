...
title: Offline-first cryptographic Proof of Delivery (PoD) with photo and signature
  on route-optimization under Zero-Trust Authentication & Token Invalidation
...
story:
  ...
  i_want: tamper-proof offline package signature capture and geolocation stamping
    for route-optimization with resilience to Zero-Trust Authentication & Token Invalidation
...
- Recipient disputing delivery when photo metadata shows GPS coordinates 50 meters
  away from address%!(EXTRA string=route-optimization) exacerbated by Zero-Trust Authentication
  & Token Invalidation
