...
story:
  ...
  i_want: strict schema registry validation with automated quarantine for unexpected
    column alterations on schema-drift-detection with resilience to Data Drift & Silent
    Schema Corruption
...
- High-frequency column renames causing silent data loss if mapping rules rely on
  strict name equality%!(EXTRA string=schema-drift-detection) exacerbated by Data
