...
story:
  ...
  i_want: strict schema registry validation with automated quarantine for unexpected
    column alterations on lakehouse-compaction with resilience to Idempotency & Replay
...
- High-frequency column renames causing silent data loss if mapping rules rely on
  strict name equality%!(EXTRA string=lakehouse-compaction) exacerbated by Idempotency
