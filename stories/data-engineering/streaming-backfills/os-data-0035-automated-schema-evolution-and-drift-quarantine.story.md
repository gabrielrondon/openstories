...
story:
  ...
  i_want: strict schema registry validation with automated quarantine for unexpected
    column alterations on streaming-backfills with resilience to Multi-Tenant Data
...
- High-frequency column renames causing silent data loss if mapping rules rely on
  strict name equality%!(EXTRA string=streaming-backfills) exacerbated by Multi-Tenant
