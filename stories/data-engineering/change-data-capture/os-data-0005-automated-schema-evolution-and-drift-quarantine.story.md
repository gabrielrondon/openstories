...
story:
  ...
  i_want: strict schema registry validation with automated quarantine for unexpected
    column alterations on change-data-capture with resilience to Multi-Tenant Data
...
- High-frequency column renames causing silent data loss if mapping rules rely on
  strict name equality%!(EXTRA string=change-data-capture) exacerbated by Multi-Tenant
