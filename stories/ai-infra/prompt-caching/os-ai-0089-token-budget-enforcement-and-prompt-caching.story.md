...
- scenario: Cache invalidation due to dynamic timestamp injected in system prompt
    for prompt-caching combined with Zero-Trust Authentication & Token Invalidation
...
- Provider cache eviction during low-traffic night hours causing unexpected latency
  spikes%!(EXTRA string=prompt-caching) exacerbated by Zero-Trust Authentication &
  Token Invalidation
