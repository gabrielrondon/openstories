...
- scenario: Zip+4 boundary spanning two different county tax rates for payments-and-checkout
  ...
  when: The tax calculation engine resolves the street address%!(EXTRA string=payments-and-checkout)
...
- B2B customers presenting tax exemption certificates that have expired or belong
  to a different state%!(EXTRA string=payments-and-checkout) exacerbated by Zero-Trust
...
- Does the implementation mitigate b2b customers presenting tax exemption certificates
  that have expired or belong to a different state%!(extra string=payments-and-checkout)
